(function () {
  ...
  const STORAGE_KEY = config.storageKey || 'desk-tidy-guided-lesson';
  ...
  function downloadPdf() {
    ...
    const fileName = `${(state.studentName || 'student').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}-${(config.title || 'desk-tidy').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}.pdf`;
